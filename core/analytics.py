...
def record_snapshot(connection: sqlite3.Connection, snapshot: AnalyticsSnapshot) -> int:
    ...
    raw = json.dumps(snapshot.raw_payload, ensure_ascii=False, sort_keys=True)[:100_000]
...
def aggregate_publication(connection: sqlite3.Connection, publication_attempt_id: int, window: str = "lifetime", *, emit_feedback: bool = False) -> dict[str, Any] | None:
    ...
    row = connection.execute(
        """SELECT s.*, pa.channel AS publication_channel FROM analytics_snapshots s
           ...
           WHERE s.publication_attempt_id=? AND s.window=?
           ORDER BY s.collected_at DESC,s.id DESC LIMIT 1""",
        (publication_attempt_id, window),
    ).fetchone()
    if row is None:
        return None
    ...
    performance = {
        ...
        "provider": row["provider"], "channel": row["publication_channel"] or row["channel"], "window": window,
        "metrics": metrics,
    ...
    with connection:
        connection.execute(
            """INSERT INTO content_performance(publication_attempt_id,snapshot_id,provider,channel,window,views,impressions,unique_views,clicks,reactions,likes,comments,shares,saves,engagements,engagement_rate,click_rate)
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
            ...
            (publication_attempt_id, row["id"], row["provider"], row["channel"], window,
        ...
        connection.execute(
            """INSERT INTO content_feedback(publication_attempt_id,channel,performance_window,performance_score,score_components)
               VALUES (?,?,?,?,?) ON CONFLICT(publication_attempt_id,performance_window,generated_at) DO NOTHING""",
            (publication_attempt_id, performance["channel"], window, score, json.dumps(components, ensure_ascii=False)),
        )
        if emit_feedback:
